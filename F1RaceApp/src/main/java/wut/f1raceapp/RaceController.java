...
public class RaceController implements Initializable, FlagObserver {
    ...
    public void switchToWeather() throws IOException {
        ...
        if(this.weatherScene == null) {
            // Tworzymy nową scenę jeśli przełączamy się pierwszy raz

            FXMLLoader fxmlLoader = new FXMLLoader(RaceApp.class.getResource("weather-scene.fxml"));
            Scene sceneWeather = new Scene(fxmlLoader.load());
            this.weatherScene = sceneWeather;

            sceneWeather.getStylesheets().add(RaceApp.class.getResource("styles/main-styles.css").toExternalForm());

            WeatherController wcontroller = fxmlLoader.getController();
            wcontroller.setDataStorage(this.dataStorage); // Dodajemy ten sam dataStorage
            wcontroller.setMainScenePrev(mainScene);

            Stage stage = (Stage) weatherButton.getScene().getWindow();
            stage.setScene(sceneWeather);
            stage.show();
        }
        else {
            Stage stage = (Stage) weatherButton.getScene().getWindow();
